      ...
      <div className="space-y-6 py-2">
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-4 p-4 rounded-xl border transition-all text-left ${
              type === 'fixed_per_service' 
              ? 'border-accent bg-accent/5 ring-1 ring-accent' 
              : 'border-border hover:border-border-strong bg-background-secondary/50'
            }`}
          >
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${type === 'fixed_per_service' ? 'bg-accent text-background-primary' : 'bg-background-tertiary text-text-tertiary'}`}>
              <Briefcase size={20} />
            </div>
            <div className="flex-1">
              <p className={`text-sm font-bold ${type === 'fixed_per_service' ? 'text-text-primary' : 'text-text-secondary'}`}>Pago Fijo por Servicio</p>
              <p className="text-xs text-text-tertiary">Valor fijo para el barbero según el servicio.</p>
            </div>
            {type === 'fixed_per_service' && <Badge variant="success">Seleccionado</Badge>}
          </button>
        </div>
