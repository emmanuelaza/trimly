...
export function NuevaCitaModal({ isOpen, onClose, onCreated, barberId, barbershopId }: Props) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    getBarberServices(barbershopId).then(setServices);
  }, [isOpen, barbershopId]);

  useEffect(() => {
    if (searchTimeout.current) clearTimeout(searchTimeout.current);
    if (!clientSearch.trim() || clientSearch.length < 2) { setClients([]); return; }
    searchTimeout.current = setTimeout(() => {
      searchBarberClients(barbershopId, clientSearch).then(setClients);
    }, 300);
  ...
  const reset = () => {
    ...
    setSelectedService(null); setDate(todayStr()); setTime(nextHourStr()); setNotes('');
  };
  ...
  const handleCreateClient = async () => {
    ...
    const res = await createBarberClient(barbershopId, barberId, session.token, newClientName, newClientPhone || null);
    if (!res.success) { toast.error('Error al crear cliente'); return; }
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    const res = await createBarberAppointment(barbershopId, barberId, session.token, {
    ...
    if (!res.success) {
      toast.error(res.error === 'slot_taken' ? 'Ya tienes una cita en ese horario' : 'Error al crear la cita');
      return;
    }
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <div className="space-y-2">
              ...
              {selectedClient ? (
                <div className="flex items-center justify-between bg-accent/10 border border-accent/30 rounded-xl px-4 py-3">
                  ...
                  <button onClick={() => { setSelectedClient(null); setClientSearch(''); }} className="text-text-tertiary hover:text-text-primary">
                    <X size={16} />
                  </button>
                </div>
              ) : (
                <div className="relative">
                  ...
                  <input type="text" placeholder="Buscar cliente por nombre..." value={clientSearch}
                  ...
                  {clients.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-background-secondary border border-border rounded-xl shadow-lg z-10 overflow-hidden">
                      {clients.map((c) => (
                        <button key={c.id} onClick={() => { setSelectedClient(c); setClientSearch(c.name); setClients([]); }}
              ...
              {!selectedClient && (
                <button onClick={() => setShowNewClientForm((v) => !v)}
              ...
              {showNewClientForm && (
                <div className="bg-background-tertiary rounded-xl p-4 space-y-3">
                  <input type="text" placeholder="Nombre *" value={newClientName}
                  ...
                  <input type="tel" placeholder="Teléfono" value={newClientPhone}
                ...
                {services.map((s) => (
                  <button key={s.id} onClick={() => setSelectedService(s)}
                    className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border transition-all ${
                      selectedService?.id === s.id ? 'border-accent bg-accent/10' : 'border-border bg-background-tertiary hover:border-border-strong'
                    }`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {selectedService?.id === s.id && <Check size={16} className="text-accent" />}
                    </div>
                  </button>
                ))}
                {services.length === 0 && (
            ...
            <div className="space-y-1">
              ...
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
                placeholder="Ej: cliente pide corte tipo militar..." rows={2}
