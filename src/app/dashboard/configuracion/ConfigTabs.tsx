...
import { Save, Building2, Clock, Bell, User2, Scissors, ChevronDown, Smartphone, Copy, ExternalLink, MessageCircle, DollarSign, CreditCard, Check, Flame } from 'lucide-react';
...
export default function ConfigTabs({ data }: { data: { barbershop: any, services: any[], barbers: any[] } }) {
  ...
  function TabPlanes() {
    ...
    const planMeta = { label: 'Licencia Trimly', icon: <Flame size={18} />, precio: '$399.000 pago único', cls: 'bg-accent/10 text-accent' };
    const features = [
      'Barberos ilimitados', 'Citas ilimitadas', 'Reportes avanzados',
      'Automatizaciones completas', 'Nómina y comisiones', 'Soporte prioritario',
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-xs text-text-secondary">Tu licencia actual y estado de activación.</p>
        </div>
        ...
        {/* Current plan card */}
        <Card className="p-5">
          <p className="text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-4">
            {isActive ? 'Licencia activa' : 'Licencia'}
          </p>
